...
namespace MagicLeap.SetupTool.Editor
{
    ...
    public class ApplyAllRunner
    {
        ...
        private void ApplyAll()
        {
            ...
            if (!_stepsToComplete[_currentApplyAllState].IsComplete)
            {
            ...
            if (_stepsToComplete[_currentApplyAllState].Block)
            {
        ...
        public static void CheckLastAutoSetupState()
        {
            ...
            if (MagicLeapPackageUtility.HasMagicLeapSDKInstalled && EditorPrefs.GetInt(string.Format(CURRENT_INPUT_SYSTEM_PREF, EditorKeyUtility.GetProjectKey()), 0) != (int)UnityProjectSettingsUtility.InputSystemType)
            {
