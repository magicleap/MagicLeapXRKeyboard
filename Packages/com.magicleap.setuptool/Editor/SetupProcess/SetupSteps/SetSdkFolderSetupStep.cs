...
namespace MagicLeap.SetupTool.Editor.Setup
{
    ...
    public class SetSdkFolderSetupStep : ISetupStep
    {
        ...
        /// <inheritdoc />
        public bool IsComplete => MagicLeapPackageUtility.HasRootSDKPath;
        ...
        public bool Draw()
        {
            ...
            else
            {
                if (CustomGuiContent.CustomButtons.DrawConditionButton(new GUIContent(LOCATE_SDK_FOLDER_LABEL),
                                                                       ...
                                                                       new GUIContent(UPDATE_SDK_FOLDER_BUTTON_LABEL, _sdkRoot), Styles.FixButtonStyle, false))
                {
        ...
        public void ChangeSDK()
        {
            ...
            var directorPath = GetCurrentSDKLocation();

            var path = EditorUtility.OpenFolderPanel(SDK_FILE_BROWSER_TITLE, directorPath, GetCurrentSDKFolderName());
            if (path.Length != 0)
            {
                SetRootSDK(path);
            }
        ...
        private void SetRootSDK(string path)
        {
            ...
            else
            {
                ...
                Debug.Log($"[{MagicLeapPackageUtility.SdkRoot}] | "
                      + $"{ !string.IsNullOrEmpty(MagicLeapPackageUtility.SdkRoot) } | {Directory.Exists(MagicLeapPackageUtility.SdkRoot)} | {Directory.Exists(MagicLeapPackageUtility.SdkRoot)}  | {File.Exists(Path.Combine(MagicLeapPackageUtility.SdkRoot, ".metadata", "sdk.manifest"))}");
