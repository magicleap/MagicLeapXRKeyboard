...
namespace MagicLeap.SetupTool.Editor.Utilities
{
    ...
    public static class UnityProjectSettingsUtility
    {
        ...
        {
            var window = EditorWindow.GetWindow(Type.GetType("UnityEditor.SettingsWindow,UnityEditor.CoreModule"));
            if (window == null)
            {
                Debug.LogWarning(CANNOT_FIND_PROJECT_SETTINGS_WINDOW);
                return;
            }
            ...
            if (info == null)
            {
                Debug.LogWarning(CANNOT_FIND_SELECT_PROVIDER_IN_PROJECT_SETTINGS);
                return;
            }
